	 ...
	 class view {
		...
		function __construct($db){
			...
			function pelanggan_data($id){
				...
				$row->execute($idx);
				$hasil = $row -> fetchAll();
				return json_decode(json_encode($hasil));
			}
			function pelanggan($query = ""){
				if($query!=""):
					$xq = "%$query%";
					$sql = "SELECT * FROM pelanggan WHERE name LIKE ? OR alamat LIKE ? OR telepon LIKE ?";
					$row = $this-> db -> prepare($sql);
					$row->execute(array($xq,$xq,$xq));
				else:
			...
			function penjualan(){
				$sql ="SELECT penjualan.* , barang.id_barang, barang.nama_barang, member.id_member,
						pelanggan.name as nmpelanggan,
						member.nm_member from penjualan 
					   left join barang on barang.id_barang=penjualan.id_barang 
					   left join member on member.id_member=penjualan.id_member
					   inner join pelanggan on pelanggan.id=penjualan.pelanggan_id
