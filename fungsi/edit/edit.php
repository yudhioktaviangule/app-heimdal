...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['pelanggan'])){
		$post = $_POST;
		$pelanggan = [];
		$sql = "UPDATE pelanggan SET name=?,alamat=?,telepon=? WHERE id=?";
		$isOK = true;
	...
	?>
		...
		<?php foreach($hasil1 as $hasil){?>
			<tr>
				...
				<td>
				<a href="fungsi/tambah/tambah.php?jual=jual&id=<?php echo $hasil['id_barang'];?>&id_kasir=<?php echo $_SESSION['admin']['id_member'];?>&pelanggan_id=<?=$_GET['pelanggan_id']?>" 
