...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['pelanggan'])){
		...
		if($v){
			$sql = "INSERT INTO pelanggan(name,alamat,telepon)VALUES(?,?,?)";
			$row = $config->prepare($sql);
	...
	if(!empty($_GET['jual'])){
		...
		if($hsl['stok'] > 0)
		{
			...
			$data1[] = $jumlah;
			$data1[] = $total;
			$data1[] = $_GET['pelanggan_id'];
			
			$sql1 = 'INSERT INTO penjualan (id_barang,id_member,jumlah,total,pelanggan_id) VALUES (?,?,?,?,?)';
